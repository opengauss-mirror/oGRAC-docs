...
- **PARALLEL n**: 指定重建索引时的并行度。

    - n的取值范围为[1, 64]。
    - 函数索引不支持并行重建。

- **PCTFREE n**: 指定为索引数据块保留的空间百分比。当数据块的可用空间低于该空间百分比时，只能更新该数据块的数据，不能向该数据块插入数据。

    - n的取值范围为[0, 80]，默认值是8。

- **tablespace_name**: 表空间名，支持重建索引到其他表空间。

- **INITRANS n**: 修改索引初始数据页面上事务槽的个数。
