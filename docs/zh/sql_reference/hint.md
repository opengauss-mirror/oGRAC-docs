...
| 类别 | 语法 | 说明 |
| ---- | ---- | ---- |
| 优化器方法 | RULE | 提示优化器使用RBO模式 |
| 路径访问 | FULL | 指示优化器对指定的表使用全表扫描 |
| 路径访问 | INDEX | 提示对指定的表使用指定的索引扫描 |
| 路径访问 | INDEX_ASC | 提示优化器对指定的表使用升序索引扫描 |
| 路径访问 | INDEX_DESC | 提示优化器对指定的表使用降序索引扫描 |
| 路径访问 | NO_INDEX | 提示优化器不要对指定的表使用一个或多个索引。 |
| 关联顺序 | LEADING | 指示优化器使用指定的表顺序作为执行计划中的前缀 |
| 关联顺序 | ORDERED | 提示优化器按照from子句中的表的顺序连接表 |
| 关联方式 | USE_NL | 提示优化器将指定的表作为被驱动表，以嵌套循环连接的方式将每个指定的表连接起来。 |
| 关联方式 | USE_HASH | 提示优化器将指定的表作为被驱动表，以哈希连接的方式将每个指定的表连接起来。 |
| 关联方式 | USE_MERGE | 提示优化器将指定的表作为被驱动表，以归并连接的方式将每个指定的表连接起来。 |
| 其它 | OPT_PARAM | 提示优化器使用此参数替换系统参数执行此sql语句。 |
...
> **说明：** 使用RULE提示会强制优化器使用基于规则的优化器来执行该语句。但是，如果SQL语句中使用了基于规则的优化器不支持的构造（例如，某些类型的连接或索引），则可能无法使用该提示。
...
> **说明：** 这个提示只对所指定的表进行全表扫描，而不是查询中的所有表。
