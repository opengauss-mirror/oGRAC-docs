...
- 会话级
  - SERIALIZABLE：事务隔离的最高级别，可实现事务之间的完全隔离。
  - READ COMMITTED：默认隔离级别，可确保不会读取到脏数据；单个 SQL 语句内读取的数据为同一版本的快照数据。
  - CURRENT COMMITTED：单个 SQL 语句内读取的数据中，每一行均为读取时刻最新的已提交数据，读取结果不再是快照数据。
- 全局级
  - DB_ISOLEVEL：配置事务隔离级别，确保不会读取到未提交的脏数据。
  - RC：Read Committed，即读已提交，单个 SQL 语句内读取的数据为同一版本的快照数据，该级别为默认隔离级别。
  - CC：Current Committed，即当前已提交，单个 SQL 语句内读取的每一行数据均为读取时刻最新的已提交数据，读取结果不再是快照数据。
