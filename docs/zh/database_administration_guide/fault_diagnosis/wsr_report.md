...
1. WSR snap_id1 snap_id2
    ...
    报告文件名由系统自动生成，是最基础的生成报告命令，当不需要自定义报告文件名时使用。

2. WSR snap_id1 snap_id2 "FILENAME"

    含义：使用两个指定的快照ID生成一份WSR性能报告，并自定义报告的文件名和保存路径。

    可指定路径（如 "e:\wsr.html"）将报告保存到特定位置。

3. WSR starttime endtime

    含义：直接指定一个时间范围（而不是快照ID）来生成性能报告。

    时间格式必须为 "yyyy-mm-dd hh24:mi:ss"。当需要分析一个非常具体的时间段（如已知的应用卡顿时间段），但该时间段没有精确的快照对应时使用。
...
4. WSR list
    ...
    快速查看最近有哪些可用的快照，以便选择正确的快照ID生成报告。
...
6. CALL WSR$CREATE_SNAPSHOT;
    ...
    在需要进行性能分析前（如准备进行压力测试），或应用突然变慢时，手动打一个快照点，以便与之后的情况做对比。
...
8. CALL WSR$DROP_SNAPSHOT_RANGE(10, 20);
    ...
    删除指定范围内的快照（此示例为删除从快照ID 10 到 20 的所有快照），清理不再需要的历史快照数据，释放存储空间。操作需谨慎，因为数据删除后无法恢复。
