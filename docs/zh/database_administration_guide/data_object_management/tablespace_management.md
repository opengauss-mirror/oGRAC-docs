...
- `TEMP2`：存放`NOLOGGING`表的数据
- `TEMP2_UNDO`：存放`NOLOGGING`表的Undo数据
...
- `DATAFILE 'DATA_FILE_NAME' SIZE FILE_SIZE`标识指定的数据文件的名称，其初始文件大小为`FILE_SIZE`。
- `AUTOEXTEND ON NEXT EXTEND_SIZE`设置自动扩展，即表空间数据插满`FILE_SIZE`后自动扩展`EXTEND_SIZE`。

示例：

创建一个名为`OGRACSPACE`的表空间，一个`EXTENTS`包含`16`个`PAGE`，指定其数据文件为`OGRACSPACE1`，数据文件的初始大小为`128M`，并设置自动扩展`64M`。
