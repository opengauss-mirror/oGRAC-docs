...
oGRAC 的表空间层级结构可以分为 `TableSpace->Segment->Extent->Page->Row`。
表空间（`TableSpace`）由多个段（`Segment`）组成，常见的段类型有数据段、索引段、回滚段等。`Segment` 的下一层级为 `Extent`，然后是 `Page`，默认 `1 extent = 8 Page`。页是数据库磁盘管理的最小操作单元，oGRAC 默认页大小为 `8K`。
查询当前数据库的 `page size`：
...
按需触发页面压缩：oGRAC 使用页面压缩（`page compact`）处理空闲空间，页面压缩的触发时机有两种：
