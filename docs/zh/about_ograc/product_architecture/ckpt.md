...
当全量CKPT时，数据库需要将所有被修改过的数据页（脏页）一次性、全部写入磁盘。对于一个繁忙的数据库，脏页可能非常多。这种集中的、大量的磁盘 I/O 会瞬间占用大量系统资源，导致数据库在 CKPT 期间性能急剧下降，出现"卡顿"现象。这带来的另一个问题就是CKPT不能频繁触发，导致point推进不及时，宕机恢复时间变长。增量 CKPT 就是为了解决这些问题而生的。

oGRAC增量CKPT的核心思想是：不再一次性刷写所有脏页，而是通过一种机制，持续地、分批次地将脏页写入磁盘。这样，任何一个时间点需要持久化的脏页数量都大大减少，从而平滑了I/O写入，避免了性能尖峰。
