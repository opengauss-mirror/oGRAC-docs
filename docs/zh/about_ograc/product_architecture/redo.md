...
一个写操作在不同的页面记录了Redo/Undo，在记录间隙数据库发生掉电，在重新拉起回滚未提交的事务时找不到想要的日志，就会导致数据不一致。因此，oGRAC 实现了日志的原子化操作，在一个原子操作内，要么都做，要么都不做，实现了Redo/Undo的原子落盘。

buffer_enter_page时会对页面添加排他锁，其他连接无法读写该页面，当buffer_leave_page时，页面锁会降级为共享锁，此时其他连接可以读取该页面。只有当原子操作结束时，锁才会全部释放。
