...
oGRAC(openGauss Real Application Cluster)是openGauss的多主数据库架构，它通过将集群中的各数据库实例管理起来并支持全局实时一致的读写操作，提升集群的读写吞吐能力、扩展能力和高可用能力。其核心思想是：数据库提供便于扩展成多主架构的内核，数据库实例间通过分布式内存服务（DMS）实现跨节点的事务、页面缓存一致性，各数据库实例共享一份数据存储，以实现各节点同时读写操作、各节点共享存储。
